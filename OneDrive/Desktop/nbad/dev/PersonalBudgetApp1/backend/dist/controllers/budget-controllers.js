...
export const sendBudgetToUser = async (req, res, next) => {
    try {
        // const newBudget = {
        ...
        //   misc:req.body.misc
        // }
        let _user = await User.findById(res.locals.jwtData.id);
        _user.budget.push(req.body.eatout);
        const user = await User.findByIdAndUpdate(res.locals.jwtData.id, { budget: _user.budget }, { new: true });
        if (!user) {
...
//# sourceMappingURL=budget-controllers.js.map